...
* реестр `KindId`, `StreamId`, `AlgoId`, `CursorId`, а также конкретные операции `op_id` в `host.ctrl`.
...
3.1.3. При ошибке `task.init` должно возвращать один из кодов `panic`, `invalid` или `rejected`. В этом случае:
...
3.1.4. Создание run-а иным способом, кроме `task.init`, запрещено.
...
#### 4.2.1. Управляющие операции: `host.ctrl`

```c
host.ctrl(
    target_run_id, // RunId, к которому применяется управляющее действие
    op_id,         // идентификатор операции управления (опаковое значение)
    params_blob    // бинарный блок параметров операции
) -> code;
```

4.2.1.1. `host.ctrl` представляет собой абстракцию операций control-plane над run-ами и их окружением.

4.2.1.2. Конкретные виды операций (`op_id`) и их влияние на планирование, ресурсы и конфигурацию задаются выше по стеку и должны при этом:

* сохранять инварианты состояний (§2.3);
* не нарушать инварианты кодов завершения (§2.4);
* не изменять `code` run-а без явного перехода в `DONE`.
...
6.7. **Расширяемость без нарушения ядра**: добавление новых типов run-ов (`KindId`), потоков (`StreamId`), алгоритмов (`AlgoId`), курсоров (`CursorId`) и управляющих операций (`op_id` в `host.ctrl`) допускается без изменения данной спецификации при условии соблюдения всех перечисленных инвариантов.
...
1. По §3.1.4 создание run-а допускается только через `task.init`.
...
* бинарные блоки (`params_blob`, `event_blob`, `out_snapshot`, `out_item`);
...
Так как глобальных ресурсов, требующих единолочного владения всеми run-ами, в ABI не описано, соблюдение данной спецификации гарантирует возможность реализации без глобальных блокировок, влияющих на всю систему целиком. ∎
...
1. **TASK (`task.init`, `task.done`).**
   Удаление `task.init` лишает систему единственного механизма создания run-ов (§3.1.4), что делает невозможным существование динамических единиц работы. Попытка перенести создание в другие ABI-вызовы нарушит инвариант единственности источника run-ов (§6.1).
...
2. **HOST (`host.ctrl`, `host.sync`).**
   ...
   Удаление `host.ctrl` вынуждает инкапсулировать все управляющие операции в другие ABI-вызовы или вне их, что разрушает разделение control-plane и data-plane и делает невозможным локализованное управление `RunId` без нарушения изоляции и согласованности (§5.5, §6.5).
